...
ComponentContainer::ComponentContainer(Circuit *circuit, QGraphicsItem *parent): //Need to come up with a new name that isnt component or circuit, circuit is too general and does not represent a circuit's component but CircuitComponent feels too long..., once you find it, consider renaming pin and body with the prefix of it such as circuitBody and circuitPin
    ...
    statePinList_{new QList<GraphicsPin*>{}}, //consider making the pin lists not dynamically allocated
    outPinList_{new QList<GraphicsPin*>{}},
    circuit_{circuit}
{
    generateStatePins();
    generateOutPins();
...
void ComponentContainer::generateStatePins() {
    qreal interval {body_->boundingRect().height()/circuit_->inSize()};
    ...
    for (int i {0}; i < circuit_->inSize(); i++) {
        statePinList_->append(new GraphicsPin{topLeft.x(), topLeft.y() + interval*i, constant::PIN_DIAMETER, constant::PIN_DIAMETER, GraphicsPin::State, this});
    }
}
...
void ComponentContainer::generateOutPins() {
    ...
    for (int i {0}; i < circuit_->outSize(); i++) {
        outPinList_->append(new GraphicsPin{topRight.x(), topRight.y() + interval*i, constant::PIN_DIAMETER, constant::PIN_DIAMETER, GraphicsPin::Out, this});
    }
}
